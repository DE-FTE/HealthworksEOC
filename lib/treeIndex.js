...
// Blob on cold starts.
//
const BLOB_INDEX_PREFIX = 'hw-tree-index/';

/**
 * Save the tree index payload to Vercel Blob (no-op when not on Vercel).
...
export async function saveTreeIndexToBlob(docId, payload) {
  if (process.env.PDF_SOURCE !== 'vercel-blob') return;
  try {
    const { put } = await import('@vercel/blob');
    await put(
      `${BLOB_INDEX_PREFIX}${docId}.json`,
      JSON.stringify(payload),
      { access: 'public', contentType: 'application/json', addRandomSuffix: false }
    );
  } catch (err) {
    console.warn('[treeIndex] Blob save failed:', err.message);
  }
}

/**
 * Ensure the tree index is in /tmp — fetches from Blob if not cached locally.
...
export async function ensureTreeIndex(docId) {
  ...
  if (process.env.PDF_SOURCE !== 'vercel-blob') return;  // local dev: /tmp is persistent
  try {
    const { list } = await import('@vercel/blob');
    const blobPath = `${BLOB_INDEX_PREFIX}${docId}.json`;
